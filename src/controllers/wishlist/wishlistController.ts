...
export const addToWishlist = async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (req.user) {
        ...
        if (!wishlist.items.some((item) => item.product instanceof mongoose.Types.ObjectId 
        ...
        return res.status(200).json({ success: true, message: "Product added to wishlist (Guest)" });
      }
    } catch (error) {
      res.status(500).json({
        success: false,
...
export const removeFromWishlist = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user) {
      ...
      if (!req.session.wishlist) {
        return res.status(404).json({ success: false, message: "Wishlist not found (Guest)" });
      }
...
export const clearWishlist = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(200).json({ success: true, message: "Wishlist cleared successfully (Guest)" });
  } catch (error) {
    res.status(500).json({
      success: false,
